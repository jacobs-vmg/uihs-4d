...
var _sheetVisitors_es = "Visitors-es";
var _sheetComments_es = "Comments-es";
...
$('#signInForm, #commentForm').submit(function(event){
    ...
    if (_id == "commentForm"){
        if ($(this).hasClass('es')){
            // _data._type = "comment_es"
            _data.sheet = _sheetComments_es;
        } else {_data.sheet = _sheetComments;}
       _data._type = "comment"
    } else {
        if ($(this).hasClass('es')){
            _data.sheet = _sheetVisitors_es;
            // _data._type = "signIn_es"
        } else { _data.sheet = _sheetVisitors; }  
        _data._type = "signIn"
    }
...
function apiCall(user, endpoint){
    ...
    if (localStorage.getItem("userLog") === null){
        var arr = [];
        var userLog = JSON.stringify(arr);
        localStorage.setItem("userLog", userLog)
    } 
        var restoreUserLog = localStorage.getItem("userLog") 
        var arr = JSON.parse(restoreUserLog);
        arr.push(user.arr);
        var userLog = JSON.stringify(arr);
        localStorage.setItem ("userLog", userLog)
